...
class TestJWKSFetcher:
    async def test_fetch_and_cache_keys(self, jwks_response, test_kid, monkeypatch):
        handler, call_count = _make_mock_transport(jwks_response)
        fetcher = JWKSFetcher("http://test/.well-known/jwks.json", cache_ttl=3600)
    ...
    async def test_cache_prevents_refetch(self, jwks_response, test_kid, monkeypatch):
        handler, call_count = _make_mock_transport(jwks_response)
        fetcher = JWKSFetcher("http://test/.well-known/jwks.json", cache_ttl=3600)
    ...
    async def test_unknown_kid_triggers_refresh(self, jwks_response, test_kid, monkeypatch):
        handler, call_count = _make_mock_transport(jwks_response)
        fetcher = JWKSFetcher(
    ...
    async def test_rate_limiting(self, jwks_response, test_kid, monkeypatch):
        handler, call_count = _make_mock_transport(jwks_response)
        fetcher = JWKSFetcher(
            "http://test/.well-known/jwks.json",
    ...
    async def test_fetch_failure_logged(self, test_kid, monkeypatch, caplog):
    ...
    async def test_malformed_jwk_skipped(self, test_kid, monkeypatch):
        jwks_with_bad_key = {
            "keys": [
                {"kty": "invalid", "kid": "bad-key"},
                # We can't easily test this without a real valid key alongside
            ]
        }
        handler, _ = _make_mock_transport(jwks_with_bad_key)
        fetcher = JWKSFetcher("http://test/.well-known/jwks.json")
    ...
    async def test_empty_jwks_response(self, monkeypatch):
        handler, _ = _make_mock_transport({"keys": []})
        fetcher = JWKSFetcher("http://test/.well-known/jwks.json")
