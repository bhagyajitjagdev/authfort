...
:::note
Auto-created users have no password and cannot use email/password login directly. They can add a password later via `set_password()` or the forgot-password flow — see [Password Management](/authfort/server/password-management/). This flag is independent of `allow_signup` — you can set `allow_signup=False` to block email/password registration while still allowing passwordless sign-up.
:::
