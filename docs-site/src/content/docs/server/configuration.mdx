...
export const configExample = `from authfort import AuthFort, CookieConfig, GoogleProvider
...
export const providersExample = `from authfort import GoogleProvider, GitHubProvider
...
The `AuthFort` constructor accepts the following parameters:
...
RS256 (RSA + SHA-256) is the only supported algorithm. It uses asymmetric keys — the auth server signs with a private key, and microservices verify with the public key from JWKS.
