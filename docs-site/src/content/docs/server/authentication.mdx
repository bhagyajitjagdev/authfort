...
if isinstance(result, MFAChallenge):
    # User has MFA enabled — prompt for TOTP code then:
    auth_response = await auth.complete_mfa_login(result.mfa_token, totp_code)
else:
    # Normal login — result.tokens.access_token, result.tokens.refresh_token
    pass`;
...
If the user has **TOTP MFA enabled**, the endpoint instead returns `MFAChallenge` (`mfa_required: true`, `mfa_token`, `expires_in: 300`). The client must POST the token + code to `POST /auth/mfa/verify` to receive full tokens. See [TOTP MFA](/authfort/server/mfa/) for the complete setup and login flow.
...
Add TOTP MFA (Google Authenticator, Authy) on top of password login:

- [TOTP MFA](/authfort/server/mfa/) — setup flow, backup codes, posture checks
