...
If the user has **TOTP MFA enabled**, the endpoint instead returns `MFAChallenge` (`mfa_required: true`, `mfa_token`, `expires_in: 300`). The client must POST the token + code to `POST /auth/mfa/verify` to receive full tokens. See [TOTP MFA](/authfort/server/mfa/) for the full flow.
