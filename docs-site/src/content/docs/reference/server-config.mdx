...
| `mfa_verify` | `str \| None` | `"5/min"` | MFA verification endpoint (`/auth/mfa/verify`) |
...
| `mfa_issuer` | `str \| None` | App name shown in authenticator apps (defaults to `jwt_issuer`) |
| `mfa_backup_code_count` | `int` | Number of backup codes generated per user (default 10) |
