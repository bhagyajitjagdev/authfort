...
3. **Run migrations**
   ...
   <CodeBlock title="Terminal" lang="bash" code='authfort migrate --database-url "sqlite+aiosqlite:///auth.db"' />
...
Exceeding the limit returns 429 with a `Retry-After` header. See [Rate Limiting](/authfort/server/rate-limiting/) for details.
...
- [Rate Limiting](/authfort/server/rate-limiting/) — Protect endpoints from abuse
- [User Management](/authfort/server/user-management/) — List, search, and delete users
